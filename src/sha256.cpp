...
uint32_t sha256::Sigma0(uint32_t value) {
  return rightRotate(value, 2) ^ rightRotate(value, 13) ^
         rightRotate(value, 22);
}

uint32_t sha256::Sigma1(uint32_t value) {
  return rightRotate(value, 6) ^ rightRotate(value, 11) ^
         rightRotate(value, 25);
}

std::vector<uint32_t>
sha256::computeMessageSchedule(const std::vector<uint8_t> &block) {
  std::vector<uint32_t> messageSchedule(64);
  for (unsigned long i = 0; i < 16; ++i) {
    messageSchedule[i] = static_cast<unsigned int>(
        (block[i * 4] << 24) | (block[i * 4 + 1] << 16) |
        (block[i * 4 + 2] << 8) | block[i * 4 + 3]);
  }
  for (unsigned long i = 16; i < 64; ++i) {
    messageSchedule[i] =
        sigma1(messageSchedule[i - 2]) + messageSchedule[i - 7] +
        sigma0(messageSchedule[i - 15]) + messageSchedule[i - 16];
  }
